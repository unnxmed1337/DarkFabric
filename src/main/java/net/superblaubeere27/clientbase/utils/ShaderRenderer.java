...
public class ShaderRenderer {
    ...
    public ShaderRenderer(String shaderName) {
        int program = glCreateProgram();

        String shadersDir = "/assets/" + DarkFabric.getInstance().getName().toLowerCase() + "/shaders/";

        try {
            glAttachShader(program, createShader(ShaderRenderer.class.getResourceAsStream(shadersDir
                    + "passthrough.vsh"), GL_VERTEX_SHADER));
            glAttachShader(program, createShader(ShaderRenderer.class.getResourceAsStream(shadersDir
                            + shaderName.replaceAll(".fsh", "") + ".fsh"),
                    GL_FRAGMENT_SHADER));
