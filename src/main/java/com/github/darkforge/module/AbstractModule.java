...
import com.github.darkforge.module.options.ValueChoice;
...
public abstract class AbstractModule implements INameable, Listenable {
    ...
    public void toggle(boolean state) {
        if (state) {
            ...
            try {
                onEnable();
                DarkForgeReloaded.getInstance().getEventBus().subscribe(this);
            ...
            try {
                onDisable();
                DarkForgeReloaded.getInstance().getEventBus().unsubscribe(this);
            } catch (Exception ignored) {
            }
        }
    }
