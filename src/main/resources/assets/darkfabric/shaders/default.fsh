...
float fbm(vec2 n) {
    ...
    float total = 0.0, amplitude = 1.65;
    for (int i = 0; i <= 10; i++) {
...
void main() {
    ...
    const vec3 c1 = vec3(156.0/255.0, 0.0/255.0, 165.0/255.0);
    const vec3 c2 = vec3(145.0/255.0, 0.0/255.0, 145.4/255.0);
    const vec3 c3 = vec3(0.2, 0.0, 0.0);
    const vec3 c4 = vec3(174.0/255.0, 1.0/255.0, 185.4/255.0);
    const vec3 c5 = vec3(0.65);
    const vec3 c6 = vec3(0.95);



    //This is how "packed" the smoke is in our area. Try changing 8.0 to 1.0, or something else
    vec2 p = gl_FragCoord.xy * 28.0 / resolution.xx;
